// Example: REST API using the generic Logic pattern for automatic JSON
// binding and response encoding.
// 示例：使用泛型 Logic 模式实现自动 JSON 绑定和响应编码的 REST API。
//
...
func main() {
	...
	// Use generic Logic helpers — request body is auto-decoded into T,
	// response is auto-encoded as JSON.
	// 使用泛型 Logic 辅助函数 — 请求体自动解码为 T，响应自动 JSON 编码。
	ferret.GetLogic(api, "/users/{id}", getUser)
	ferret.PostLogic(api, "/users", createUser)
	ferret.DeleteLogic(api, "/users/{id}", deleteUser)
