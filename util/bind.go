...
// Validator is the interface for validating the decoded value.
// Validator 是用于校验解码后值的接口。
...
// application/x-www-form-urlencoded and multipart/form-data; when Content-Type
// is missing it falls back to form/query binding.
// Unsupported media types return a 415 error.
// Bind 类似 Gin 的 ShouldBind，根据 Content-Type 头自动将请求绑定到 v，
// 支持 application/json、application/xml、text/xml、
// application/x-www-form-urlencoded 和 multipart/form-data；
// 缺少 Content-Type 时回退为表单/查询参数绑定；
// 不支持的媒体类型返回 415 错误。
